...
// Data datang dari server (props) — tidak ada fetch di sini
export default function HeroSection({ slides = [], grid = [] }) {
    ...
    <div className="space-y-10">
      ...
      <div className="md:hidden">
        <div className="relative overflow-hidden rounded-2xl border border-[var(--border)]">
              ...
              <div key={slide.id} className={isActive ? 'block' : 'hidden'}>
                <Link href={articleHref(slide)} className="block">
                  ...
                  <div className="px-5 py-5" style={{ background: TITLE_BG }}>
                    ...
                    {slide.excerpt && (
                      <p className="mt-1.5 text-sm text-white/55 line-clamp-2">{slide.excerpt}</p>
                    )}
                    <time className="mt-3 block text-xs text-white/40" dateTime={slide.published_at}>
          ...
          {slides.length > 1 && (
            <div className="absolute bottom-[72px] right-4 z-10 flex items-center gap-1.5">
              {slides.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setActive(i)}
      ...
      <div className="hidden md:flex gap-3 items-start">
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {slide.excerpt && (
                    <p className="mt-2 text-sm text-white/70 line-clamp-2 max-w-xl">{slide.excerpt}</p>
                  )}
        ...
        <div className="grid grid-cols-2 gap-2 p-2 bg-[var(--background)] border-l border-[var(--border)]">
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-3 z-10">
                  <h3 className="font-serif text-[13px] font-semibold leading-snug text-white
                    line-clamp-3 group-hover:underline decoration-white/60 underline-offset-2">
                    {a.title}
                  </h3>
                  <time className="mt-1 block text-[10px] text-white/50" dateTime={a.published_at}>
                    {fmt(a.published_at)}
                  </time>
                </div>
              </Link>
